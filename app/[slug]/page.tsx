...
export default async function WatchPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!channel) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-6 pb-24">
        <h1 className="text-4xl font-extrabold text-gray-900 mb-4">Saluran Tidak Ditemukan</h1>
        <p className="text-gray-500 mb-8 font-medium">Maaf, siaran yang Anda cari sedang tidak aktif.</p>
        <Link href="/" className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-200">Kembali ke Beranda</Link>
      </div>
    );
  }
  ...
  // Use url_stream from JSON, with a hard fallback only if both url_stream and url are missing
  let finalUrl = channel.url_stream || channel.url || "https://rtvstream.rtv.co.id:4555/hls/rtv.m3u8";

  // Security workaround: If window is HTTPS, some browsers block HTTP streams.
  // We can't fix it if the server doesn't support HTTPS, but we can try to warn or log.
  console.log(`Loading stream for ${channel.name}: ${finalUrl}`);
      ...
      <main className="flex-1 overflow-y-auto px-6 md:px-12 pb-24 pt-8 scrollbar-hide">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-8">
            <div className="rounded-[2.5rem] overflow-hidden shadow-2xl shadow-blue-900/10 bg-black">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-white">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gray-100 pb-8 mb-8">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="px-2.5 py-1 bg-red-500 text-white rounded-lg text-[10px] font-bold tracking-wide uppercase">LIVE NOW</span>
                ...
                <div className="flex items-center gap-3">
                  <button className="flex items-center gap-2 bg-gray-50 text-gray-700 px-5 py-3 rounded-2xl hover:bg-gray-100 transition-all font-bold text-sm"><Heart className="w-5 h-5" /></button>
                  <button className="flex items-center gap-2 bg-gray-50 text-gray-700 px-5 py-3 rounded-2xl hover:bg-gray-100 transition-all font-bold text-sm"><Share2 className="w-5 h-5" /></button>
                  <button className="flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-2xl hover:bg-blue-700 transition-all font-bold text-sm shadow-lg shadow-blue-200">Langganan</button>
                </div>
              </div>

              <div>
                <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-3">
                  <PlayCircle className="w-6 h-6 text-blue-600" /> Saluran Serupa
                </h3>
          ...
          <div className="space-y-10">
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-white">
              <h3 className="text-xl font-bold text-gray-900 mb-8 flex items-center gap-3">
                <MessageCircle className="w-6 h-6 text-blue-600" /> Obrolan Langsung
              </h3>
              <div className="h-[400px] flex flex-col gap-6 overflow-y-auto pr-4 scrollbar-hide mb-8">
                 <p className="text-xs text-center text-gray-400 font-bold uppercase tracking-widest px-4">Selamat datang di obrolan VisionPro!</p>
              </div>
              <div className="relative">
                <input type="text" placeholder="Katakan sesuatu..." className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all pr-16" />
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-3">Up Next</h3>
